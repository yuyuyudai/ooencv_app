...
public class JavaCamera2ViewPlus extends CameraBridgeViewBase {
    ...
    private static final String LOGTAG = "JavaCamera2ViewPlus";
    ...
    private void createCameraPreviewSession() {
        ...
        mImageReader.setOnImageAvailableListener(new ImageReader.OnImageAvailableListener() {
            ...
            public void onImageAvailable(ImageReader reader) {
                ...
                if (BuildConfig.DEBUG && !(planes.length == 3)) {
                    throw new AssertionError("Assertion failed");
                }
                if (BuildConfig.DEBUG && !(image.getFormat() == mPreviewFormat)) {
                    throw new AssertionError("Assertion failed");
                }
        ...
        CameraCaptureSession.StateCallback callback= setCameraCaptureSessionStateCB();

        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.P) {
            setCameraDevicecreateCaptureSessionPandLater(surface,callback);
        } else {
            setCameraDevicecreateCaptureSessionBeforeP(surface,callback);
        }
    }
    ...
    private void setCameraDevicecreateCaptureSessionPandLater(Surface surface, CameraCaptureSession.StateCallback callback) {
        try {
    ...
    private void setCameraDevicecreateCaptureSessionBeforeP(Surface surface, CameraCaptureSession.StateCallback callback) {
        try {
            mCameraDevice.createCaptureSession(Collections.singletonList(surface), callback, null);
        } catch (CameraAccessException e) {
            e.printStackTrace();
        }
    }
    ...
    boolean calcPreviewSize(final int width, final int height) {
        ...
        try {
            ...
            if (BuildConfig.DEBUG && (frameSize.width == 0 || frameSize.height == 0)) {
                throw new AssertionError("Assertion failed");
            }
    ...
    private class JavaCamera2Frame implements CvCameraViewFrame {
        ...
        public Mat gray() {
            ...
            if (BuildConfig.DEBUG && !(planes[0].getPixelStride() == 1)) {
                throw new AssertionError("Assertion failed");
            }
        ...
        public Mat rgba() {
            ...
            if (chromaPixelStride == 2) { // Chroma channels are interleaved
                ...
                if (BuildConfig.DEBUG && !(planes[2].getPixelStride() == 2)) {
                    throw new AssertionError("Assertion failed");
                }
                ByteBuffer y_plane = planes[0].getBuffer();
                int y_plane_step = planes[0].getRowStride();
                ...
                if (addr_diff > 0) {
                    if (BuildConfig.DEBUG && !(addr_diff == 1)) {
                        throw new AssertionError("Assertion failed");
                    }
                ...
                if (y_plane_step == w) {
                    ...
                    if (BuildConfig.DEBUG && !(yuv_bytes_offset == w * h)) {
                        throw new AssertionError("Assertion failed");
                    }
                }
    ...
    public int getRotationIndex() {
        ...
        int devicerotation;  //0,1,2,3 mean 0,90,180,270 CW degrees
        CameraManager manager = (CameraManager) getContext().getSystemService(Context.CAMERA_SERVICE);
        try {
            CameraCharacteristics characteristics = manager.getCameraCharacteristics(mCameraID);
        ...
        devicerotation = getMyDisplay().getRotation();
